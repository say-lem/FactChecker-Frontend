...
const CheckDetails = () => {
  ...
  const activeVerdict =
    verdictFromApi.length > 0 ? verdictFromApi[0].verdict : "neutral";

  const getButtonClass = (type: string) => {
    return activeVerdict === type
      ? verdictColors[type]
      : verdictColors["neutral"];
  };

  return (
    <div className="flex justify-center relative h-[calc(100vh-64px)] overflow-hidden">
      <div
      ...
      <div className="p-4 space-y-6 container max-w-7xl absolute z-10 text-white">
        <div className="flex relative items-center pt-28">
          <div className="text-xl font-semibold">Query: {queryText}</div>
          <div
            className="flex items-center gap-2 cursor-pointer mb-6 md:mb-[70px] hover:text-gray-200 absolute top-2"
            onClick={handleBack}
          >
            <FaArrowLeft />
            <span>Back</span>
        ...
        <div className="flex gap-3 mb-4 text-white">
          <button
            className={`px-4 py-2 rounded-[8px] font-semibold ${getButtonClass(
              "positive"
            )}`}
          >
            True
